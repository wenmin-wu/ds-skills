...
510 个可复用技巧，覆盖 5 大领域，均从高票 Kaggle 竞赛方案中提取。每个技能是一个独立的 SKILL.md —— 可直接被 Claude Code、Cursor、Codex 或任何 AI 编程助手加载使用。
...
| 领域 | 数量 | 示例 |
|------|------|------|
| **tabular** (表格数据) | 113 | simulated-annealing-multi-operator, strtree-spatial-index-collision, convex-hull-bbox-rotation, squeeze-compact-local-search |
| **nlp** (自然语言处理) | 119 | lda-topic-modeling, taxonomy-context-enrichment, learned-attention-pooling, domain-special-token-embedding |
| **cv** (计算机视觉) | 170 | clip-interrogator-captioning, embedding-knn-regression, weighted-embedding-blending, classifier-free-guidance-diffusion |
| **timeseries** (时间序列) | 58 | ratio-target-for-smape, density-to-count-roundtrip, expanding-window-stacking, retroactive-outlier-rescaling |
