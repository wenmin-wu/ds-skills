...
410 个可复用技巧，覆盖 5 大领域，均从高票 Kaggle 竞赛方案中提取。每个技能是一个独立的 SKILL.md —— 可直接被 Claude Code、Cursor、Codex 或任何 AI 编程助手加载使用。
...
| 领域 | 数量 | 示例 |
|------|------|------|
| **tabular** (表格数据) | 98 | autoencoder-timeseries-embedding, nelder-mead-threshold-optimization, tabnet-sklearn-wrapper, threaded-parquet-describe-features |
| **nlp** (自然语言处理) | 113 | two-stage-retrieve-rerank, multi-retriever-union-ensemble, length-sorted-batching, dropout-disabled-inference |
| **cv** (计算机视觉) | 125 | slice-as-channel-2d-cnn, modality-adaptive-dicom-windowing, middle-mip-std-volume-projection, lateralized-label-flip-tta-disable |
...
| **llm** (大语言模型) | 26 | vllm-lora-adapter-inference, last-token-logit-binary-scoring, completion-only-lm-training, multi-gpu-process-isolated-vllm |
