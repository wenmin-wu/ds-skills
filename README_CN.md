...
370 个可复用技巧，覆盖 5 大领域，均从高票 Kaggle 竞赛方案中提取。每个技能是一个独立的 SKILL.md —— 可直接被 Claude Code、Cursor、Codex 或任何 AI 编程助手加载使用。
...
| 领域 | 数量 | 示例 |
|------|------|------|
| **tabular** (表格数据) | 88 | adversarial-validation, yearly-partitioned-groupby, sparse-dense-hstack-lgbm, cross-dataset-user-aggregation |
| **nlp** (自然语言处理) | 108 | deberta-classification, layerwise-lr-decay, keystroke-essay-reconstruction, last-hidden-states-concat |
| **cv** (计算机视觉) | 105 | two-stage-slice-localization-then-organ-crop, anisotropic-spacing-trilinear-resample, per-organ-multihead-sigmoid-softmax, per-patient-slice-maxpool-aggregation |
| **timeseries** (时间序列) | 43 | locale-scoped-holiday-flag, rolling-refit-arima-forecast, store-profile-hierarchical-clustering, neighbor-average-nan-interpolation |
