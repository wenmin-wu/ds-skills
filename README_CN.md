...
535 个可复用技巧，覆盖 5 大领域，均从高票 Kaggle 竞赛方案中提取。每个技能是一个独立的 SKILL.md —— 可直接被 Claude Code、Cursor、Codex 或任何 AI 编程助手加载使用。
...
| 领域 | 数量 | 示例 |
|------|------|------|
| **tabular** (表格数据) | 123 | hierarchical-rule-engine, opponent-avoidance-scoring, ball-landing-prediction, chained-target-prediction |
| **nlp** (自然语言处理) | 119 | lda-topic-modeling, taxonomy-context-enrichment, learned-attention-pooling, domain-special-token-embedding |
| **cv** (计算机视觉) | 182 | stroke-temporal-color-rendering, conv1d-lstm-stroke-classifier, deterministic-hash-partitioning, chunked-csv-image-generator |
| **timeseries** (时间序列) | 61 | multi-lag-target-features, periodogram-seasonality-detection, kaggle-api-streaming-inference, ratio-target-for-smape |
| **llm** (大语言模型) | 50 | perplexity-prompt-ranking, low-meaning-input-substitution, 4bit-nf4-double-quantization, inverse-task-prompt-template |
