...
535 reusable techniques across 5 domains, each extracted from top-voted Kaggle competition solutions. Every skill is a self-contained SKILL.md — ready to be loaded by Claude Code, Cursor, Codex, or any AI coding agent.
...
|--------|-------|---------|
| **tabular** | 123 | hierarchical-rule-engine, opponent-avoidance-scoring, ball-landing-prediction, chained-target-prediction |
| **nlp** | 119 | lda-topic-modeling, taxonomy-context-enrichment, learned-attention-pooling, domain-special-token-embedding |
| **cv** | 182 | stroke-temporal-color-rendering, conv1d-lstm-stroke-classifier, deterministic-hash-partitioning, chunked-csv-image-generator |
| **timeseries** | 61 | multi-lag-target-features, periodogram-seasonality-detection, kaggle-api-streaming-inference, ratio-target-for-smape |
| **llm** | 50 | perplexity-prompt-ranking, low-meaning-input-substitution, 4bit-nf4-double-quantization, inverse-task-prompt-template |
